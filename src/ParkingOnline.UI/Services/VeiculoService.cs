...
public class VeiculoService(HttpClient httpClient) : IVeiculoService
{
    ...
    {
        using HttpResponseMessage _ = await httpClient.PostAsJsonAsync("veiculos/Add", veiculoModel);
    }

    public async Task DeleteVeiculoAsync(int id)
    {
        using HttpResponseMessage _ = await httpClient.DeleteAsync($"veiculos/Delete/{id}");
    }

    public async Task<IEnumerable<VeiculoModel>> GetAllVeiculosAsync()
    {
        List<VeiculoModel>? veiculos = await httpClient.GetFromJsonAsync<List<VeiculoModel>>("veiculos/GetAll");
    ...
    {
        VeiculoModel? veiculo = await httpClient.GetFromJsonAsync<VeiculoModel>($"veiculos/GetById/{id}");
    ...
    {
        using HttpResponseMessage _ = await httpClient.PutAsJsonAsync($"veiculos/Update/{id}", veiculoModel);
    }
}