...
public class TicketService(HttpClient httpClient) : ITicketService
{
    ...
    {
        using HttpResponseMessage _ = await httpClient.PostAsJsonAsync("tickets/Add", ticketModel);
    }

    public async Task DeleteTicketAsync(int id)
    {
        using HttpResponseMessage _ = await httpClient.DeleteAsync($"tickets/Delete/{id}");
    }

    public async Task<IEnumerable<TicketModel>> GetAllTicketsAsync()
    {
        List<TicketModel>? tickets = await httpClient.GetFromJsonAsync<List<TicketModel>>("tickets/GetAll");
    ...
    {
        TicketModel? ticket = await httpClient.GetFromJsonAsync<TicketModel>($"tickets/GetById/{id}");
    ...
    {
        using HttpResponseMessage _ = await httpClient.PutAsJsonAsync($"tickets/Update/{id}", ticketModel);
    }
}