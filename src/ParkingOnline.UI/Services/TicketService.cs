...
public class TicketService(HttpClient httpClient) : ITicketService
{
    ...
    {
        using var _ = await httpClient.PostAsJsonAsync("tickets/Add", ticketModel);
    }

    public async Task DeleteTicketAsync(int id)
    {
        using var _ = await httpClient.DeleteAsync($"tickets/Delete/{id}");
    }

    public async Task<IEnumerable<TicketModel>> GetAllTicketsAsync()
    {
        var tickets = await httpClient.GetFromJsonAsync<List<TicketModel>>("tickets/GetAll");
    ...
    {
        var ticket = await httpClient.GetFromJsonAsync<TicketModel>($"tickets/GetById/{id}");
    ...
    {
        using var _ = await httpClient.PutAsJsonAsync($"tickets/Update/{id}", ticketModel);
    }
}